...
public class MagasinTest
{
    ...
    {
        CompteBancaire client = new CompteBancaire(35000);
        String nomProduits = "Playstation";
        int prixDeBase = 350;
        int stockMax = 100;
        Magasin magasin = new Magasin(nomProduits, prixDeBase, stockMax);
        magasin.acheter(90, client);
        Assertions.assertEquals(10, magasin.getStock());
        Assertions.assertEquals(35000 - 90 * 350, client.getSolde());
    ...
    /**
     * Un magasin dispose d'un compte bancaire initialisé à 100€. Un magasin dispose d'un compte bancaire initialisé à
     * 100€. Il est possible de consulter le solde du compte d'un magasin mais on ne peut pas agir directement sur le
     * compte.
     */
    @Test
    public void testMagasinCompteBancaire()
    {
        String nomProduits = "Brosses à dent";
        Magasin magasin = new Magasin(nomProduits, 2, 5000);
        Assertions.assertEquals(100, magasin.getSolde());
    }

    /**
     * La méthode acheter() de la classe Magasin doit prendre en paramètre la quantité de produits achetés ainsi que le
     * compte bancaire du client.
     * <p>
     * À l'issue de l'achat, le compte bancaire du client est débité du prix réel d'un produit, multiplié par le nombre
     * de produits achetés. Le solde du compte du magasin est augmenté de la valeur achetée par le client.
     */
    @Test
    public void testConsulterCompteBancaireMagasin()
    {
        CompteBancaire client = new CompteBancaire(1154);
        Magasin magasin = new Magasin("Brosses à dent", 2, 5000);

        int soldeClientAvantAchat = client.getSolde();
        int soldeMagasinAvantAchat = magasin.getSolde();
        int prixReelProduits = magasin.getPrixReelProduits();
        int quantite = 10;

        magasin.acheter(quantite, client);

        Assertions.assertEquals(soldeClientAvantAchat - prixReelProduits * quantite, client.getSolde());
        Assertions.assertEquals(soldeMagasinAvantAchat + prixReelProduits * quantite, magasin.getSolde());
    }
}
